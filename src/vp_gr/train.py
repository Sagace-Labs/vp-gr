...
from vp_gr import contract as gr_contract
from vp_gr import data as gr_data
from vp_gr import model as gr_model
...
def build_version(
    ...
    *,
    ...
    X = fingerprints.featurize(smiles, gr_model.FEATURES)

    fitted = {}
    for output in gr_contract.column_names():
        column = OUTPUT_LABELS[output]
        rows = gr_data.labelled(table, column)
...
def _write_version(
    ...
    *,
    ...
    labels = list(gr_data.LABELS)
    sources = ", ".join(
        f"AID {e.pubchem_aid} ({e.assay_name})" for e in (TARGET, ANTAGONIST, CYTOTOX)
    )
    record = {
        ...
        "supersedes": supersedes,
        "reason": reason,
        "signature": gr_contract.as_manifest_table(),
        "dataset": {
            "name": "Tox21 glucocorticoid receptor qHTS",
            "source": (
                f"PubChem BioAssay {sources}, rows called Active or Inactive, one "
                f"row per compound labelled by majority call across its assay records"
            ),
            "url": (
    ...
    print(
        ...
        f"  next: python -m vp_gr.evaluate --version {version}",
        file=sys.stderr,
    )
    return directory
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(prog="python -m vp_gr.train")
