"""The GR dataset: obtain, standardise, verify.
...
Three endpoints share the table. ``label`` is the agonist call and defines
which compounds the table holds. ``antagonist`` and ``cytotox`` are the calls
of the other two screens over the same library, and are null for a compound a
screen did not call.
...
from vp_gr.target import ANTAGONIST, CYTOTOX, TARGET
...
TABLE_PATH = DATA_DIR / "gr_tox21.parquet"
EXAMPLE_PATH = DATA_DIR / "example" / "gr_example.parquet"

#: The label columns the table carries, primary endpoint first.
LABELS: tuple[str, ...] = ("label", "antagonist", "cytotox")
...
def _missing(path: Path) -> str:
    return (
        ...
        "`python -m vp_gr.data fetch` to rebuild it from PubChem."
    )
...
def _to_compounds(records: pd.DataFrame, structures: pd.DataFrame) -> pd.DataFrame:
    """Collapse agonist records to one row per standardised compound.
...
def _secondary(
    records: pd.DataFrame, structures: pd.DataFrame, column: str
) -> pd.DataFrame:
    """Collapse a further screen's records the same way, keyed for a join on InChIKey."""
    df = _standardise(records, structures)
    ...
    for inchikey, group in df.groupby("inchikey"):
        ...
        rows.append(
            {
                "inchikey": inchikey,
                column: int(active_frac > 0.5),
                f"{column}_n_calls": len(group),
                f"{column}_active_frac": round(active_frac, 6),
            }
        )
...
def fetch(*, write: bool = True) -> pd.DataFrame:
    ...
    fetched = {}
    for endpoint in (TARGET, ANTAGONIST, CYTOTOX):
        print(f"fetching AID {endpoint.pubchem_aid} from PubChem", file=sys.stderr)
        fetched[endpoint.name] = _assay_records(endpoint.pubchem_aid)

    cids = sorted({cid for frame in fetched.values() for cid in frame["cid"]})
    structures = _smiles_for(cids)

    table = _to_compounds(fetched[TARGET.name], structures)
    # A left join: the agonist endpoint decides which compounds the table holds,
    # and each further screen fills in where it also has a call.
    for endpoint, column in ((ANTAGONIST, "antagonist"), (CYTOTOX, "cytotox")):
        table = table.merge(
            _secondary(fetched[endpoint.name], structures, column),
            on="inchikey",
            how="left",
        )
    ...
    for column in LABELS[1:]:
        called = table[column].notna()
        lines.append(
            f"  {column} calls {int(called.sum())} of them, "
            f"{int(table.loc[called, column].sum())} positive"
        )
    lines.append(f"  sha256 {dataset.dataset_hash(table, labels=LABELS)}")
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(prog="python -m vp_gr.data")
